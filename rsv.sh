#!/bin/sh
# rsv: Read the Revised Standard Version Bible from your terminal
...
while [ $# -gt 0 ]; do
	...
	if [ "$1" = "--" ]; then
		...
		get_data rsv.tsv | awk -v cmd=list "$(get_data rsv.awk)"
		exit
	elif [ "$1" = "-W" ]; then
		export RSV_NOLINEWRAP=1
		shift
...
if [ $# -eq 0 ]; then
	...
	while true; do
		...
		get_data rsv.tsv | awk -v cmd=ref -v ref="$ref" "$(get_data rsv.awk)" | ${PAGER}
	done
	exit 0
fi

get_data rsv.tsv | awk -v cmd=ref -v ref="$*" "$(get_data rsv.awk)" | ${PAGER}
