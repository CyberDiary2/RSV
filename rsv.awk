BEGIN {
	...
	MAX_WIDTH = 80
	if (ENVIRON["RSV_MAX_WIDTH"] ~ /^[0-9]+$/) {
		if (int(ENVIRON["RSV_MAX_WIDTH"]) < MAX_WIDTH) {
			MAX_WIDTH = int(ENVIRON["RSV_MAX_WIDTH"])
		}
	}
...
function printverse(verse,    word_count, characters_printed) {
	if (ENVIRON["RSV_NOLINEWRAP"] != "" && ENVIRON["RSV_NOLINEWRAP"] != "0") {
		printf("%s\n", verse)
		return
	}
